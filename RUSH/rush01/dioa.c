...
void solvsudoku(int row, int column)
{
    if (row > 8)
        display();
    ...
    else
    {
        ...
        for (ctr = 1; ctr <= 9; ctr++)
        {
            if ((checkrow(row, ctr) == 1) && (checkcolumn(column, ctr) == 1) &&
                (checkgrid(row, column, ctr) == 1))
            {
